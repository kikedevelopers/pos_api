...
describe('Compartir inventario entre companies (e2e, pos_db) — FASE 2', () => {
    ...
    afterAll(async () => {
        if (!ds) return;
        for (const id of created) await cleanupCompany(ds, id);
        await ds.destroy();
    });
    ...
    maybe(
        ...
        async () => {
            ...
            const p1 = await insertProduct(ds!, principal, { name: 'Compartido 1', cost: 10, stock: 100 });
            ...
            const catalogB = await findAll.execute(branch, {} as InventoryQueryDto);
            ...
            const catalogA = await findAll.execute(principal, {} as InventoryQueryDto);
    ...
    maybe(
        ...
        async () => {
            ...
            const pid = await insertProduct(ds!, principal, { name: 'Vender compartido', cost: 10, stock: 100 });
    ...
    maybe(
        ...
        async () => {
            ...
            const base = await insertProduct(ds!, principal, { name: 'Base compartida', cost: 20, stock: 1000 });
            ...
            await ds!.transaction(async (m) => {
                await adjustInventory(
                    m,
                    branch,
                    [{ item_id: Number(child), quantity: 1 }],
                    'DEDUCT',
                    { reason: 'SALE', referenceType: 'sale_invoice', crossCompanyAccess: true }
                );
            });
