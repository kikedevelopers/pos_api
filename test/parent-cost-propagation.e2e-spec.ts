...
describe('Parent → children cost propagation (e2e, pos_db)', () => {
  ...
  async function expectChildPricesRecomputed(cid: number, childId: string, childCost: number) {
    ...
    for (const p of prices) {
      ...
      const expMargin =
        p.sale > 0 ? round4(new Big(p.sale).minus(childCost).div(p.sale).times(100)) : 0;
  ...
  it('FORM: editar cost del padre propaga a hijos (cost + profit/margin + historial EDIT/PARENT)', async () => {
    ...
    const childB = await insertProduct(ds, companyId, {
      name: 'Hijo B 250',
      cost: 250,
      packagingId: pkg250,
      parentId,
    ...
    await formAction.execute(Number(parentId), { cost: 1500 }, companyId, E2E_ACTOR);
    ...
    for (const [cid, prices] of [
      ...
      expect(ph.every((r: { cost_history_id: string }) => r.cost_history_id === hist[0].id)).toBe(
        true,
      );
    }
  });
  ...
  it('EXCEL: editar cost del padre vía bulk propaga a hijos', async () => {
    ...
    const res = await bulkAction.execute(
      [
        {
          name: 'Padre EXCEL',
          sku_code: 'PADRE-EXCEL',
          cost: 2000,
          prices: [{ sale_price: 5000 }],
        },
      ],
      companyId,
      E2E_ACTOR,
  ...
  it('NO-OP: editar padre con el MISMO cost → hijos sin cambios ni historial', async () => {
    ...
    await formAction.execute(Number(parentId), { cost: 1000 }, companyId, E2E_ACTOR);
  ...
  it('editar HIJO directo (parent_id != null) → no propaga, sin filas PARENT', async () => {
    ...
    await formAction.execute(Number(childA), { cost: 600 }, companyId, E2E_ACTOR);

    expect((await productRow(companyId, childA)).cost).toBe(600);
    const hist = await costHistory(companyId, childA);
    expect(hist.filter((h: { derived_from: string }) => h.derived_from === 'PARENT')).toHaveLength(
      0,
    );
  ...
  it('padre SIN hijos → editar cost OK, sin historial PARENT', async () => {
    ...
    await expect(
      formAction.execute(Number(parentId), { cost: 1234 }, companyId, E2E_ACTOR),
  ...
  it('hijo SIN packaging → se omite (no crashea); resto de hijos sí se actualiza', async () => {
    ...
    const childWith = await insertProduct(ds, companyId, {
      name: 'Hijo CON pkg',
      cost: 500,
      packagingId: pkg500,
      parentId,
    });
    const childNo = await insertProduct(ds, companyId, {
      name: 'Hijo SIN pkg',
      cost: 300,
      packagingId: null,
      parentId,
    ...
    await expect(
      formAction.execute(Number(parentId), { cost: 1500 }, companyId, E2E_ACTOR),
    ).resolves.toBeDefined();
  ...
  it('precisión Big.js: cost padre 1333 (pkg 1000) → hijo 500 = 666.5', async () => {
    ...
    await formAction.execute(Number(parentId), { cost: 1333 }, companyId, E2E_ACTOR);
  ...
  it('multi-tenant: la propagación filtra company_id — hijo de otra company no se toca', async () => {
    ...
    const parentA = await insertProduct(ds, companyId, {
      name: 'Padre MT A',
      cost: 1000,
      stock: 5,
      packagingId: pkgParentA,
    });
    const childA = await insertProduct(ds, companyId, {
      name: 'Hijo MT A',
      cost: 500,
    ...
    await formAction.execute(Number(parentA), { cost: 1500 }, companyId, E2E_ACTOR);
